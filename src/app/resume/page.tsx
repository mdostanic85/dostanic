  ...
  return (
    <main className="resume-page bg-white text-[#111318]">
      <div className="resume-actions mx-auto flex w-full max-w-[1100px] items-center justify-between gap-4 px-5 pb-6 pt-24 sm:px-8">
        <Link href="/" className="font-mono text-[10px] uppercase tracking-[0.2em] text-[#111318]/60 hover:text-[#2742ff]">← Portfolio</Link>
        <ResumeActions />
      </div>
      ...
      <article className="mx-auto w-full max-w-[1100px] px-5 pb-24 sm:px-8">
        <header className="grid grid-cols-1 gap-10 border-y border-[#111318]/20 py-10 sm:grid-cols-12">
          <div className="sm:col-span-8">
            <p className="font-mono text-[10px] uppercase tracking-[0.24em] text-[#2742ff]">Milos Dostanic</p>
          ...
          <address className="not-italic sm:col-span-4 sm:text-right">
            ...
            <a href="https://dostanic.net" className="block text-sm leading-7 hover:text-[#2742ff]">dostanic.net</a>
          </address>
        </header>

        <section className="grid grid-cols-1 gap-10 border-b border-[#111318]/20 py-10 sm:grid-cols-12">
          <div className="sm:col-span-3">
            <h2 className="font-mono text-[10px] uppercase tracking-[0.24em] text-[#2742ff]">Experience</h2>
            ...
            {RESUME_EXPERIENCE.map((row, index) => (
              <article key={row.company} className={index > 0 ? 'border-t border-[#111318]/15 py-7' : 'pb-7'}>
                <div className="flex flex-col gap-1 sm:flex-row sm:items-baseline sm:justify-between">
                  ...
                  <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-[#111318]/55">{row.period}</p>
                </div>
        ...
        <section className="grid grid-cols-1 gap-10 border-b border-[#111318]/20 py-10 sm:grid-cols-12">
          <h2 className="font-mono text-[10px] uppercase tracking-[0.24em] text-[#2742ff] sm:col-span-3">Selected proof</h2>
          <div className="grid grid-cols-1 gap-x-8 gap-y-7 sm:col-span-9 sm:grid-cols-2">
            {SELECTED_PROOF.map((item) => (
              <article key={item.title}>
                ...
                <p className="mt-1 font-mono text-[9px] uppercase tracking-[0.17em] text-[#2742ff]">{item.label}</p>
        ...
        <section className="grid grid-cols-1 gap-10 py-10 sm:grid-cols-12">
          <h2 className="font-mono text-[10px] uppercase tracking-[0.24em] text-[#2742ff] sm:col-span-3">Core expertise</h2>
        ...
        <section className="grid grid-cols-1 gap-10 border-t border-[#111318]/20 py-10 sm:grid-cols-12">
          <h2 className="font-mono text-[10px] uppercase tracking-[0.24em] text-[#2742ff] sm:col-span-3">Education & languages</h2>
            ...
            <div>
              ...
              {RESUME_EDUCATION.map((item) => (
                <div key={item.qualification} className="mt-4 border-t border-[#111318]/15 pt-4">
                  ...
                  <p className="mt-1 font-mono text-[9px] uppercase tracking-[0.16em] text-[#111318]/50">{item.period}</p>
                </div>
              ))}
            </div>
            <div>
